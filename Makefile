...
GATEWAY_IP_NAME ?= ember-gateway-ip
GATEWAY_CERTIFICATE ?= ember-gateway-cert
GATEWAY_SSL_POLICY ?= ember-gateway-modern
GPU_TTL_HOURS ?= 3
CLUSTER_TTL_HOURS ?= 6
...
PUBLIC_GATEWAY_ENV = PROJECT_ID="$(GCP_PROJECT)" CLUSTER_NAME="$(GKE_CLUSTER)" CLUSTER_LOCATION="$(GKE_LOCATION)" GATEWAY_HOST="$(GATEWAY_HOST)" GATEWAY_IP_NAME="$(GATEWAY_IP_NAME)" GATEWAY_CERTIFICATE="$(GATEWAY_CERTIFICATE)" GATEWAY_SSL_POLICY="$(GATEWAY_SSL_POLICY)"

.PHONY: fmt tidy test vet build web-install web-test web-build replit-build replit-run scripts-check verify images manifests kind-create kind-load cluster-auth kind-auth keda-install deploy sample kind-smoke control-api-smoke undeploy gke-cluster-create gke-cluster-status gke-build-images gke-deploy gke-real-smoke gke-gateway-prepare gke-gateway-deploy gke-gateway-status gke-gateway-smoke gke-gateway-destroy gcp-cost-guard-setup gcp-cost-guard-arm gcp-cost-guard-status gcp-cost-guard-disarm gcp-cost-guard-keep-cluster gcp-cost-guard-destroy
...
manifests:
	...
	kubectl kustomize deploy/gke/public-gateway >/dev/null
...
gke-gateway-prepare:
	$(PUBLIC_GATEWAY_ENV) ./scripts/gke-public-gateway.sh prepare

gke-gateway-deploy:
	$(PUBLIC_GATEWAY_ENV) ./scripts/gke-public-gateway.sh deploy

gke-gateway-status:
	$(PUBLIC_GATEWAY_ENV) ./scripts/gke-public-gateway.sh status

gke-gateway-smoke:
	$(PUBLIC_GATEWAY_ENV) ./scripts/gke-public-gateway.sh smoke

gke-gateway-destroy:
	$(PUBLIC_GATEWAY_ENV) CONFIRM_DESTROY="$(CONFIRM_DESTROY)" ./scripts/gke-public-gateway.sh destroy
...
gcp-cost-guard-keep-cluster:
	$(COST_GUARD_ENV) ./scripts/gcp-cost-guard.sh keep-cluster
